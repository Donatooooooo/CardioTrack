...
from predicting_outcomes_in_heart_failure.data.split_data import split_one, VARIANTS, TARGET_COL
...
def test_split_one_creates_files(csv_dir, tmp_path, variant):
    ...
    with patch("predicting_outcomes_in_heart_failure.data.split_data.PROCESSED_DATA_DIR", tmp_path / "processed"):
...
# TEST: _safe_train_test_split()
from loguru import logger
from predicting_outcomes_in_heart_failure.data.split_data import _safe_train_test_split
...
@pytest.mark.parametrize("X, y, expected_classes, log_level, log_text", [
    (
