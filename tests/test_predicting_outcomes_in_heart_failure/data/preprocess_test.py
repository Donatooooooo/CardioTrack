...
def mock_paths(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(preprocess_module, "NUM_COLS_DEFAULT", 
                        ["Age", "RestingBP", "Cholesterol", "MaxHR", "Oldpeak"])

    return tmp_path


# TEST: save_scaler_artifact()
from predicting_outcomes_in_heart_failure.data.preprocess import save_scaler_artifact
...
# TEST: generate_gender_splits()
from predicting_outcomes_in_heart_failure.data.preprocess import generate_gender_splits
...
class TestGenerateGenderSplits:
    ...
    def test_no_sex_column_in_input(self, mock_paths, sample_df_without_sex):
        ...
        assert not (mock_paths / "female.csv").exists() or \
               len(pd.read_csv(mock_paths / "female.csv")) == 0
...
# TEST: preprocessing
from predicting_outcomes_in_heart_failure.data.preprocess import preprocessing
...
class TestPreprocessing:
    ...
    @pytest.mark.parametrize("sex_input,sex_expected", [
        ("M", 1),
        ("F", 0),
    ])
    def test_preprocessing_encodes_sex(self, mock_paths, raw_heart_data, 
    ...
    @pytest.mark.parametrize("exercise_input,exercise_expected", [
        ("Y", 1),
        ("N", 0),
    ])
    def test_preprocessing_encodes_exercise_angina(self, mock_paths, raw_heart_data,
